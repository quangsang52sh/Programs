...
kneaddata --input "$input_fastq" --output "$kneaddata_output" \
    --reference-db kneaddata_reference_db

# Step 5: Analyze OTUs with Qiime2 (for 16S rRNA data)
echo "Step 5: Analyzing OTUs with Qiime2..."
qiime dada2 denoise-single --i-demultiplexed-seqs "$output_trimmed" \
...
metaphlan "$kneaddata_output/filtered.fastq" --input_type fastq \
...
blastn -query query_sequence.fasta -db "$blastn_db" \
    -out blastn_results.txt -outfmt "6 qseqid sseqid pident length evalue"
